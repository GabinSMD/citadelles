...
public class TestArchitecte {
	...
	public void test3(){
		System.out.println("TEST DE L'UTILISATION DU POUVOIR (AVATAR)");
		// On crée un plateau et on ajoute des cartes Quartier à la pioche:		
		PlateauDeJeu plateau = new PlateauDeJeu();
		Quartier quartier1 = new Quartier("temple",Quartier.TYPE_QUARTIERS[0],1);
		Quartier quartier2 = new Quartier("prison",Quartier.TYPE_QUARTIERS[1],2);
		...
		Test.test(architecte.getJoueur().nbQuartiersDansMain() == 0, "test du nombre de cartes dans la main avant l'utilisation du pouvoir = 0");			
		architecte.utiliserPouvoirAvatar();
		Test.test(architecte.getJoueur().nbQuartiersDansMain() == 2, "test du nombre de cartes dans la main après l'utilisation du pouvoir += 2");
