...
public class TestJoueur {
	...
	public void test1(){
		...
		Test.test(joueur.getNom().equals("Billy"),"test du nom du joueur constructeur");
		Test.test(joueur.nbPieces() == 0,"test du tresor initial du joueur");
		...
		Test.test(joueur.getCite().length==8,"test de la taille de la Cite");
		Test.test(!joueur.getPossedeCouronne(),"test de la couronne");
		Test.test(joueur.getMain().size()==0,"test de la taille de la main");
		Test.test(joueur.getPersonnage()== null, "test de l'attribution du personnage");
		Test.test(!joueur.getAvatar(), "test du joueur non avatar");
	}
	
	public void test2() {
		System.out.println("TEST DU JOUEUR");
		Joueur joueur = new Joueur("Billy");
		Roi roi = new Roi();
		roi.setJoueur(joueur);
		Test.test(!joueur.getAvatar(), "test du statut avatar avant definition");
		joueur.setAvatar(true);
		Test.test(joueur.getAvatar(),"test du statut avatar après definition");
		Test.test(joueur.getPersonnage()==roi,"test du personnage");
		Test.test(joueur.getNom().equals("Billy"),"test du nom du joueur");
		Test.test(!joueur.getPossedeCouronne(),"test de la couronne avant attribution");
		joueur.setPossedeCouronne(true);
		Test.test(joueur.getPossedeCouronne(),"test de la couronne après attribution");
	}
	
	public void test3() {
		System.out.println("TEST DU TRESOR");
		Joueur joueur = new Joueur("Billy");
	...
	public void test4() {
		...
		Test.test(joueur.nbQuartiersDansCite() == 2 && retour==quartier2, "test de retrait d'un quartier");
		Test.test(!joueur.quartierPresentDansCite("marche"), "test de non présence d'un quartier");
	}
	...
	public void test5() {
		...
		Quartier quartier2 = new Quartier("prison",Quartier.TYPE_QUARTIERS[1],2);
		Quartier quartier3 = new Quartier("palais",Quartier.TYPE_QUARTIERS[2],5);
		Quartier quartier4 = new Quartier("quartier de trop",Quartier.TYPE_QUARTIERS[2],5);
		
		joueur.ajouterQuartierDansMain(quartier1);
		joueur.ajouterQuartierDansMain(quartier2);
		joueur.ajouterQuartierDansMain(quartier3);
		Test.test(joueur.nbQuartiersDansMain() == 3,"test de l'ajout de trois quartiers avant cité pleine");
		joueur.ajouterQuartierDansMain(quartier1);
		joueur.ajouterQuartierDansMain(quartier2);
		joueur.ajouterQuartierDansMain(quartier3);
		joueur.ajouterQuartierDansMain(quartier1);
		...
		joueur.ajouterQuartierDansMain(quartier4);
		Test.test(joueur.getMain().get(7).getNom()!="quartier de trop","test de l'ajout d'un quartier impossible lorsque cité pleine");
		Quartier retour = joueur.retirerQuartierDansMain();
		Test.test(joueur.nbQuartiersDansMain() == 8 && (retour==quartier1 || retour==quartier2 || retour==quartier3), "test de retrait d'un quartier");		
	}
	...
	public void test6() {
		...
		joueur.ajouterPieces(2);
		joueur.reinitialiser();
		Test.test(joueur.nbQuartiersDansMain()==0 && joueur.nbQuartiersDansCite()==0 && joueur.nbPieces()==0,"test de la réinitialisation");		
	}
}